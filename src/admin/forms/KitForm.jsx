...
function KitForm() {
  ...
  const removeImage = (index) => {
    ...
    // If removed image was main, reset to 0
  ...
  const setAsMain = (index) => {
    ...
    // Move chosen image to front in both arrays
    setImages(prev => {
      const arr = [...prev];
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: '12px' }}>
              {imagePreviews.map((preview, index) => (
                ...
                >
                  ...
                  {/* Main badge */}
                  {index === 0 && (
                    <span style={{ position: 'absolute', top: '4px', left: '4px', background: '#00e5ff', color: '#000', padding: '2px 8px', borderRadius: '4px', fontSize: '10px', fontWeight: '800', letterSpacing: '0.5px' }}>
                      ⭐ MAIN
                    </span>
                  )}
                  {/* Hover hint for non-main images */}
                  {index !== 0 && (
                    <div style={{
                      position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.55)',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      opacity: 0, transition: 'opacity 0.2s',
                      fontSize: '11px', color: '#fff', fontWeight: '700', textAlign: 'center', padding: '4px'
                    }}
